          ...
          <div id="content-header" class="clearfix">
            <div class="pull-left">
              <ol class="breadcrumb">
                ...
                <li class="active"><span>Translators</span></li>
              </ol>
              <h1>Translators</h1>
            </div>
          </div>
                  ...
                  <div class="row">
					...
					<div class="table_out">
                      <table id="datatable" class="table table-striped table-bordered">
                        ...
                        <tbody>
							<?php						
							$select="select *,`u_details`.`login_id` AS `t_id` from `u_details` LEFT OUTER JOIN `u_login` ON `u_details`.`login_id`=`u_login`.`id` where `u_login`.`type`='TR'";						
							...
							while($res=mysqli_fetch_array($que)){							
								...
								echo"<tr>						
								<td>$count</td>					
								<td><a href='translator-details.php?id={$res['t_id']}'>{$res['f_name']}  {$res['l_name']}</a></td>                       
								<td>{$res['address']}</td>                       
								<td>{$res['email_id']}</td>";						
								...
								if($res['admin_doc_veri_status']==2){								  						
								echo" <td >Verified</td>";								  				
								} 							
								else							
								{								
								...
								if($res['admin_status']=='A'){								  						
									echo" <td><a href='admin_code/update_status.php?tra_id={$res['t_id']}'><button style='background-color:red; color:white; padding: 4px 4px;' >Deactive</button></a></td>";							  						
								} 							
								else							
								{								
									echo" <td><a href='admin_code/update_status.php?tra_id={$res['t_id']}'><button style='background-color:green; color:white; padding: 4px 4px;'>Active</button></a></td>";
								}   
								echo"<td><a href='admin_code/delete.php?tra_id=".$res['t_id']."'><i class='fa fa-trash'></i></a></td>";		
								echo"</tr>";					
							}												
						?>

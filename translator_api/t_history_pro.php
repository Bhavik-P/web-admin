...
if($_SERVER['REQUEST_METHOD'] == "GET"){
	...
	if($tra_id==''){
		...
		$select_a="select * from `pro_u_awarded` where tra_id='$tra_id' AND `running_status`='CM' OR 'CL'";
		$que1=mysqli_query($con,$select_a);
		while($res1=mysqli_fetch_array($que1)){
			$pro_id=$res1['pro_id'];
			
			$select_pro="select * from `project` where `id`='$pro_id' AND `running_status`='CM' OR 'CL'";
